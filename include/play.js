...
module.exports = {
  async play(song, message) {
    ...
    if (!song) {
      ...
      .setColor("#a97f4d");
      PlayEmbed.setTitle("**A fila de músicas terminou.**", true)
 
      return message.channel.send(PlayEmbed).catch(console.error);
    }
    ...
    collector.on("collect", (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        case "🔁":
          ...
          queue.textChannel.send(`Repetição agora ${queue.loop ? "**on**" : "**off**"}`).catch(console.error);
          break;
